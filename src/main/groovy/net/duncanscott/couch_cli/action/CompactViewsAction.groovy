...
class CompactViewsAction extends AbstractAction {
    ...
    String getDescription() {
        'Compact all views in databases at the specified couchdb.  Must specify individual databases via --name or --all.'
    }
	...
	int performAction() {
        ...
        databasesOption.requireDatabases(config).each { String database ->
            api.listDesignDocuments(config.couchdb.url, database).each { String designDocName ->
                message "compacting views for ${config.couchdb.name}.${database}/${designDocName}"
                api.compactViews(config.couchdb.url,database,designDocName)
            }
		}
