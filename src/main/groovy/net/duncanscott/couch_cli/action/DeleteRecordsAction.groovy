...
class DeleteRecordsAction extends AbstractAction {
    ...
    String getDescription() {
        'Delete records in databases at the specified couchdb.  Must specify individual databases via --name or --all.  CAREFULL.'
    }
	...
	int performAction() {
        ...
        CouchClient.checkDeleteRecords(config)
        ...
        databasesOption.requireDatabases(config).each { String database ->
			api.doForAllRecords(config.couchdb.url, database, includeDocs, pageSize) { json ->
				message "deleting ${config.couchdb.name}.${database}/${json.id}"
				api.deleteRecord(config.couchdb.url, database, json.id, json.'value'.'rev')
			}
		}
